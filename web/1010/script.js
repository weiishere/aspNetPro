...
(function (exp) {
    ...
    //全局变量
    (function () {
        exp.ApiUrl = "http://open.siigee.com";
        exp.ApiUrl2 = "http://m.siigee.com";
    ...
    var initPages = {
        ...
        gameOver: function () {
            ...
            if (param.type === "1") {
                //战帖
                $.ajax({
                    url: ApiUrl2 + "/game.do?action=fightManage", data: getParam({
                        "gameScore": score1.value + "",
                        "playTime": (new Date()).Format("YYYYMMDDHHmmSS"),
                        "memberMobile": param.memberMobile + "",
                        "type": param.type,
                        "gameType": param.gameType,
                        "fightId": param.fightId
                    }), type: "POST", async: false,
                    jsonp: "callback", dataType: "jsonp",
                    success: function (data2) {
                        if (data2.data.state == 0) {
                            //Trip("对不起，战帖数据更新成功~", 500);
                        } else {
                            Trip("对不起，战帖数据更新失败~", 500);
                        }
                    }
                });
                /*
                LAjax.post(ApiUrl + "/api/game/getPlayerInfo", getParam({
                    'mid': param.mid
                }), function (data1) {
                    if (data1.data.state == 0) {
                        //战帖
                        LAjax.post(ApiUrl2 + "/api/game/fightManage", getParam({
                            "gameScore": score1.value,
                            "playTime": (new Date()).Format("YYYYMMDDHHmmSS"),
                            "memberMobile": data1.data.obj.memberMobile,
                            "type": param.type,
                            "gameType": param.gameType,
                            "fightId": param.fightId
                        }), function (data2) {
                        ...
                        Trip("对不起，用户信息获取失败~", 500);
                    }
                }, function (status) {
                    alert("request error:" + status.readyState);
                });*/
            } else {
                //正常游戏
                //获取用户信息
                $.ajax({
                    url: ApiUrl + "/api/game/getPlayerInfo", data: getParam({ mid: param.mid }), type: "POST", async: false,
                    jsonp: "callback", dataType: "jsonp",
                    success: function (data1) {
                        if (data1.data.state == 0) {
                            $.ajax({
                                url: ApiUrl + "/api/game/insertGameLog", data: getParam({
                                    "mid": param.mid,
                                    "gameScore": score1.value,
                                    "playTime": (new Date()).Format("YYYYMMDDHHmmSS"),
                                    "memberMobile": data1.data.obj.memberMobile,
                                ...
                                success: function (data2) {
                                    if (data2.data.state == 0) {
                                        //更新会员积分
                                        //$.ajax({
                                        //    url: ApiUrl + "/api/game/updateMemberScore", data: getParam({ mid: param.mid, score: score1.value }), type: "POST", async: false,
                                        //    jsonp: "callback", dataType: "jsonp",
                                        //    success: function (data3) {
                                        //        if (data3.data.state == 0) {
                                        //            //Trip("获取积分~", 500);
                                        //        } else {
                                        //            Trip("对不起，更新会员积分失败~", 500);
                                        //        }
                                        //    }
                                        //});
                                    } else {
                                        Trip("对不起，插入用户记录失败~", 500);
                                    }
                                }
                            });
                        } else {
                ...
                LAjax.post(ApiUrl + "/api/game/getPlayerInfo", getParam({
                    ...
                    if (data1.data.state == 0) {
                        //插入游戏记录
                        LAjax.post(ApiUrl + "/api/game/insertGameLog", getParam({
                            "mid": param.mid,
                            "gameScore": score1.value,
                            "playTime": (new Date()).Format("YYYYMMDDHHmmSS"),
                            "memberMobile": data1.data.obj.memberMobile,
                            ...
                            if (data2.data.state == 0) {
                                //更新会员积分
                                LAjax.post(ApiUrl + "/api/game/updateMemberScore", getParam({
                                    "mid": param.mid,
                                    "score": score1.value
                                }), function (data3) {
                                    if (data3.data.state == 0) {
            ...
            /*
            var params = "{ 'MemberID': " + getParameterByName("MemberID") + ", 'Score': score1.value, 'Type': '9','timer':'" + parseInt(Math.random() * 10000) + "' }";//ChallengeScore挑战分数
        ...
        init: function () {
            ...
            eventBackLayer.addEventListener(LMouseEvent.MOUSE_UP, function (e) {
                ...
                if (!gameCommon.testState(activeBlock)) {
                    playSound("s_dropInFail", function () { });
                    ...
                    if (score_x.length != 0 || score_y.length != 0) {
                        playSound("s_getScore", function () { });
                        //document.getElementById('s_getScore').play();
                    } else {
                        playSound("s_dropIn", function () { });
