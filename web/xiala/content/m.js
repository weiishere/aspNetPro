...
(function (exp) {
    ...
    exp.requestAjax = function (url, option, data, success) {
        ...
        if ($("#loading").size() == 0) {
            $("body").append("<div id=\"loading\"><div><img src=\"http://img10.360buyimg.com/cms/jfs/t2821/84/923092587/781/ad9c511d/572b0127N566e3610.gif\" /><br />加载中，请稍后...</div></div>");
        }
        $.ajax({
            type: _option.type,
    ...
    exp.Trip = function (option) {
        ...
        window.setTimeout(function () { floatWindow.addClass("showFloat").css({ "height": floatWindow.height() * 1.1 }, 200, function () { setting.callBack(); }); }, 50);
        var _close = function () { if (floatWindow.size() != 0) { floatWindow.removeClass("showFloat").css({ "height": $("#floatWindow").height() * 0.99 }, 150, function () { $(this).remove(); $(".pageLoading2").remove(); }); } }
            ...
            if (window.hasOwnProperty("Mobilebone")) {
                Mobilebone.callback = function (pagein, pageout) {
                    ...
                    getPage(pagein.id, function () {
                        $("#" + pagein.id + " content").height($("#" + pagein.id).height());
                        this.showBack();
                        this._showCall();
