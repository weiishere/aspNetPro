window['wxprocess'] = function(){
	...
	(function(){
		...
		requestForJsonP(  
			...
			function (data) {
				...
				if(data.dataList.length){
					for(var i = 0; i < 10; i++){
						compensateHtml += '<td><p><img src="img/compensate.png" alt=""><span>已报销5元</span></p></td>';

					// for(var i = 0; i < data.dataList.length; i++){
						//compensateHtml += '<td><p><img src="'+(data.dataList[i].picUrl || baseLogo)+'" alt=""><span>已报销'+data.dataList[i].compensate+'元</span></p></td>'
					}
				}
				console.log(compensateHtml)

				// $('.listTabTr').html(compensateHtml);
				// changeListWidth();
			}
		)
		...
		var compensateHtml = '';					
					for(var i = 0; i < 10; i++){
						var td =$('<td><p><img src="img/compensate.png" alt=""><span>已报销5元</span></p></td>');
					// for(var i = 0; i < data.dataList.length; i++){
						//compensateHtml += '<td><p><img src="'+(data.dataList[i].picUrl || baseLogo)+'" alt=""><span>已报销'+data.dataList[i].compensate+'元</span></p></td>'
					$('.listTabTr').append(td);
					}
